...
Portable workflows for scaffolding, endpoint work, upgrades, and observability live in [`agent-skills/`](agent-skills/). Install them with `./agent-skills/install.sh --target <product-dir> [--claude] [--codex] [--copy]` or `make install-skills TARGET=<product-dir>`; without harness flags, the installer uses existing `.claude/` and `.agents/` parents.
