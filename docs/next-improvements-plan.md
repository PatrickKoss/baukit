...
**Status:** Complete. Shipped as v0.3.0 on 2026-09-04. Claude orchestrates; codex agents (gpt-5.6-sol, high) implement and verify. Progress lives in the execution tracker below. Commits go straight to `main`; no pull requests, and the release is prepared locally for manual publishing.
...
- [x] 14. `.env` reconciliation script (batch 4)
...
- Item 3, Leitbild: remove `cache_presented_principal` in `leitbild-api/src/lib.rs` and the local rate-limit connection checks in `leitbild-bin/src/lib.rs`. Eigenruhe: remove `auth::authenticate` and the split composition in `eigenruhe-api/src/lib.rs` and the Redis store-selection glue in `eigenruhe-bin/src/bin/api.rs`.
- Items 8 and 9, Tiefgang: delete `mobile/src/sync/rejected-push-race.test.ts` and the overlapping-response cases in `mobile/src/db/repository-contract.ts` once its adapters implement the atomic outcome callback. Redemut: delete `packages/sync/src/scheduler-environments.ts` and its test; use the browser entry and `onRecoverySignal`.
...
- 2026-09-04: v0.3.0 published manually. Tag `v0.3.0` (51054e6) is on origin, all 18 `@baukit/*` packages are on npm at 0.3.0, and all 16 crates are on crates.io at 0.3.0. Plan complete; what remains is product-side adoption from the follow-ups list above.
