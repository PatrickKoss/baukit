# Baukit implementation task archive — Phase 3 local platform (Focus I) + Focus RL

Archived 2026-08-11 from [implementation-tasks.md](./implementation-tasks.md).
Contains, verbatim, the completed Focus I waves (I0–I3, the local-first pivot
waves L1/L1b/LD/L2/L3, the pivot continuation M1–M5, and the leitbild adoption
I5a) and the completed Focus RL waves (RL1, RL2, RL4), plus their wave-log
entries. Still active — and therefore *not* here: Wave RL3 (one open live-proof
item), Wave R (parked real-environment work), Wave I10 (optional backlog), and
both focuses' architecture decisions. One repair was made while archiving: the
2026-08-11 Wave RL4 log entry had lost the start of its header line in the
source file; it is reconstructed below as "**Wave RL4 done (2 ∥ codex agents +
codex live verifier, orchestrator release).**".

## Focus I completed waves (task lists verbatim)
...
- [ ] **User action:** install the hosted Renovate GitHub App on baukit, platform-infra, leitbild, architecture-health-platform (and the three migrated products); confirm free-tier coverage for private personal repos (I0 verified the claim) *(still open — tracked under Wave R in the active file)*
...
## Focus RL completed waves (task lists verbatim)
...
## Log (archived wave-log entries)

- 2026-08-11: **Wave RL4 done (2 ∥ codex agents + codex live verifier, orchestrator release).** RL4a:
