...
CI=1 corepack pnpm --dir "$example_dir" exec expo start --dev-client --port 8081 >"$artifacts/metro.log" 2>&1 &
metro_pid=$!
metro_deadline=$((SECONDS + 120))
until curl --fail --silent http://127.0.0.1:8081/status | grep -q 'packager-status:running'; do
...
adb shell am force-stop dev.baukit.sqliteconformance
adb shell monkey -p dev.baukit.sqliteconformance -c android.intent.category.LAUNCHER 1 >/dev/null
