...
```bash
cargo install --git https://github.com/PatrickKoss/baukit --tag v0.1.0 --locked baukit-cli
...
The `@baukit/*` packages are on npm and the sixteen library crates are on crates.io at `0.1.0`. The CLI installs from a Git tag. Crates, packages, and templates move together under one `vX.Y.Z` tag.
