...
- [~] Commit the pending baukit working tree as clean logical commits (platform
  lifecycle layer, chart `testing` fix, alloy multi-env logs, template Dockerfile,
  renovate preset, docs)
- [~] Focus J library fixes that belong in this release: `testing` accepted by the
...
- [~] Version 0.3.0 → 0.3.1 coherent everywhere (`scripts/check-version-coherence.py`);
  full gate: `make ci`, tests `--include-ignored`, generated fixture (all flavors),
  metric-names lint, cargo deny if dependencies moved
- [~] Tag `baukit-v0.3.1`, push main + tag to `github.com/PatrickKoss/baukit`
- [~] platform-infra: local cluster consumes baukit via the GitHub tag (read-only
...
- [ ] Hostname-based TLS via the local CA chain (`local-ca` ClusterIssuer from L1):
...
- [ ] leitbild served over TLS; PKCE smoke green through the TLS hostname
- [ ] Ops listeners verified unreachable through the ingress (external probe, not
  values inspection)
- [ ] Dead-man/watchdog proof local: Watchdog alert routed to a small in-cluster
  heartbeat receiver; silence-means-page proven once by breaking the route
- [ ] CNPG restore-test staleness alert demonstrably fires when the CronJob is
  suspended, resolves when resumed
- [ ] Runbook/docs updates from what was actually done
...
- [ ] leitbild adopts `baukit-v0.3.1`; the `staging` bridge dies (environment
  identity `testing` end-to-end)
- [ ] Flagger 1.44.0 as a platform base (direct Traefik provider), composed by the
  local overlay; the LD MetricTemplates with the thresholds above
- [ ] Canary resource for `leitbild-api` per env (weights above); gate runner wired;
  `leitbild-primary`/`-canary` Services owned by Flagger as designed in LD/L2
- [ ] Proof both ways: a good release promotes through the full weight ladder; a
  deliberately bad release (failing gate or metrics) auto-rolls back, with evidence
  that customer traffic was unaffected
- [ ] `docs/progressive-delivery.md` + runbooks updated to reflect reality
