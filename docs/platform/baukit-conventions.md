...
- Rust crates: `baukit-runtime`, `baukit-config`, `baukit-http`, `baukit-ops`, `baukit-telemetry`, `baukit-openapi`, `baukit-test` (crate names are registry-independent and used from day one).
- npm packages: `@baukit/analytics-core`, `@baukit/analytics-posthog-web`, `@baukit/analytics-posthog-native`, `@baukit/api-runtime`, `@baukit/data-contracts`, `@baukit/ui-tokens`. These names live in `package.json` from day one; while private, products consume them as git dependencies with no registry involved, so scope ownership never comes into play until the public npmjs release (see releases below).
- CLI binary: `baukit`.
