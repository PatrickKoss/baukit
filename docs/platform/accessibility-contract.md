...
Honor the web reduced-motion media query and `AccessibilityInfo.isReduceMotionEnabled()` on native, including preference changes during a running session when the product supports them. `useReducedMotion` reads both and follows in-session changes. Remove non-essential motion, use an immediate or short cross-fade alternative, and never make animation the only signal of a state change. Essential progress and spatial context may remain, but must avoid unnecessary looping, parallax, flashing, or large movement.
...
The remaining `qa-*` specs cover what axe cannot: `qa-keyboard` asserts modal focus entry, Tab containment, Escape, focus restoration, that no tab stop lands on `[inert]` or `[aria-hidden]` content, and that keyboard focus draws a ring at least 3px wide with at least 2px offset that differs from mouse focus. `qa-overlay-dismiss` asserts every overlay closes without committing. `qa-submit-guards` asserts a double activation produces one result and a failed submit keeps input and moves focus to the invalid field. `qa-scroll` asserts the last content of a route is reachable at every viewport.
