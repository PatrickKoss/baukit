...
20. **2026-08-09 — Phase 3 GitOps architecture:** Cluster inventory is `local` (k3d — k3s-in-Docker on the dev machine, disposable), `testing` (small Hetzner k3s node, sized from measured local-stack footprint), and `production` (Hetzner k3s node per §7.2); all single-node, deliberately non-HA; PostHog remains off-cluster on its dedicated server (decision 10). **Two-layer GitOps split:** generic, secret-free platform component bases (cluster-baseline, cert-manager, Traefik, kyverno, kured, CNPG + postgres-cluster convention, Keycloak, observability stack) live in baukit under `deploy/platform/` as independently includable Flux-native kustomize bases, versioned by the release train and publishable at the Phase 4 go-public decision; everything carrying identity — domains, ACME e-mail, cloud/DNS credentials, SOPS secrets, cluster inventory, app registrations, terraform — lives in the private `platform-infra` repo (§14), which is never open-sourced. Flux CD reconciles all clusters, consuming baukit bases via a `GitRepository` pinned to a `baukit-v*` tag (read-only deploy key) so platform upgrades ride the release train; secrets are SOPS+age with per-cluster keys. **App manifests live in the app repo** (`deploy/gitops/<env>/`: HelmRelease + values + pinned image tag); platform-infra only registers apps (one read-only GitRepository + one Kustomization per environment). **Promotion is git-based:** the local release script builds/pushes images and bumps the testing tag in one commit; production is promoted by a script-assisted PR. Kargo and Flux image-automation are explicitly deferred (revisit at ≥3 deployed apps or measured promotion friction). **Renovate is the hosted Mend GitHub App** on all private repos (covers Flux/Helm/Docker/Cargo-git-tag/pnpm updates, which Dependabot cannot), never self-hosted; its PRs are validated locally per the no-hosted-CI rule. DNS records are terraform-managed initially with external-dns as an optional later base; the registry is GHCR with pull secrets and tag pruning, quota watched rather than assumed. Implementation plan: [implementation-tasks.md](./implementation-tasks.md) Focus I.

21. **2026-08-09 — Single-node ingress edge:** Do not buy a Hetzner Cloud Load Balancer for the initial testing or production node. Run the Flux-managed Traefik DaemonSet directly on host ports 80/443 (without broad `hostNetwork`) and keep its Service non-`LoadBalancer`. The current German LB11 price is €7.49/month or €0.012/hour excluding VAT, while the one application node remains the availability bottleneck, so a managed LB cannot make this deliberately non-HA cluster available when that node fails. The saving is material at this scale; the trade-off is that the public node/Primary IP is the edge and a node rebuild must preserve or reattach that IP (or update DNS), with no managed frontend health checking or future multi-node target failover. Revisit the CCM/LB base when a second ingress node or a private-only node edge is introduced. Current Volumes cost €0.0572/GB-month excluding VAT (10 GB minimum, therefore €0.572/month); they remain appropriate for PVCs but are not backups. Sources: [live Hetzner LB11 price API](https://website-price-api.hetzner.com/api/v1/products/CLOUD_25), [Hetzner Load Balancers](https://www.hetzner.com/cloud/load-balancer/), [current Volume price](https://cloudtally.eu/blog/hetzner-cloud-billing-explained), [Hetzner Volume limits and redundancy](https://docs.hetzner.com/cloud/volumes/overview/), and [15 June 2026 price adjustment scope](https://docs.hetzner.com/general/infrastructure-and-availability/price-adjustment/).

22. **2026-08-09 — Keycloak installation and realm-as-code:** Use Keycloak's official, namespace-scoped Operator at `26.7.1`, installed OLM-less from the versioned `keycloak-k8s-resources` Kustomize resources; do not use the preview cluster-wide mode. Bitnami is excluded because post-August-2025 current catalog content requires authenticated Broadcom registry access and the legacy public images are frozen. The maintained codecentric `keycloakx` chart remains a viable fallback (chart `7.2.2`, Keycloak `26.6.4`), but the official Operator now gives the preferred upstream lifecycle and provenance. Realm-as-code has an important boundary: `KeycloakRealmImport` creates a new realm once and explicitly does not update or delete it. I1b will use a Git-owned `RealmRepresentation` plus `KeycloakRealmImport` for initial bootstrap, and subsequent realm changes must be explicit, versioned Admin API migration Jobs until a separately evaluated reconciliation tool is adopted; merely changing the import CR is not reconciliation. Sources: [Keycloak 26.7.1 release](https://github.com/keycloak/keycloak/releases/tag/26.7.1), [Operator installation](https://www.keycloak.org/operator/installation), [realm import behavior](https://www.keycloak.org/operator/realm-import), [Broadcom's Bitnami catalog change](https://knowledge.broadcom.com/external/article/408744/change-to-consume-marketplace-helm-chart.html), and [codecentric keycloakx chart](https://artifacthub.io/packages/helm/codecentric/keycloakx).

23. **2026-08-09 — CloudNativePG backup API:** Pin CloudNativePG `1.30.0` (operator chart `0.29.0`) and build I1b backups on `plugin-barman-cloud` `0.14.0`, not the in-tree `.spec.backup.barmanObjectStore` integration. The native integration is deprecated; its removal was postponed from 1.30 to 1.31, which is transition time rather than a reason to create new debt. The reusable cluster component will define a `barmancloud.cnpg.io/v1` `ObjectStore`, reference `barman-cloud.cloudnative-pg.io` from `Cluster.spec.plugins` with `isWALArchiver: true`, and use `ScheduledBackup.spec.method: plugin`, preserving physical backups, WAL archiving, and PITR through the supported CNPG-I path. Sources: [CloudNativePG 1.30 release](https://github.com/cloudnative-pg/cloudnative-pg/releases/tag/v1.30.0), [1.30 release notes](https://cloudnative-pg.io/docs/1.30/release_notes/v1.30/), [Barman Cloud plugin 0.14.0](https://github.com/cloudnative-pg/plugin-barman-cloud/releases/tag/v0.14.0), and [plugin concepts](https://cloudnative-pg.io/plugin-barman-cloud/docs/concepts/).

24. **2026-08-09 — Prometheus stack packaging:** Use `kube-prometheus-stack` chart `88.2.0` (Prometheus Operator bundle app `v0.93.0`) instead of composing Prometheus, Grafana, Alertmanager, kube-state-metrics, and node-exporter charts independently. The umbrella chart gives one tested dependency set and owns the `ServiceMonitor`/`PrometheusRule` CRDs that baukit's existing observability content consumes. On a tight single-node cluster, keep one replica, disable Thanos and HA behavior, disable monitors/rules for k3s control-plane components that are not exposed, and bound resources and retention. Individual charts could avoid the Operator's resident cost, but would add manual CRD, discovery, rules, and upgrade wiring; resource measurement in I2 remains the go/no-go check. Sources: [kube-prometheus-stack chart index](https://prometheus-community.github.io/helm-charts/index.yaml) and [chart composition/configuration](https://github.com/prometheus-community/helm-charts/tree/main/charts/kube-prometheus-stack).

25. **2026-08-09 — Grafana chart repository and monolithic-mode correction:** Loki and Tempo charts moved from `https://grafana.github.io/helm-charts` to `https://grafana-community.github.io/helm-charts`; Alloy remains in the former repository. Pin Loki chart `18.7.6` (app `3.7.6`) with `deploymentMode: Monolithic`, `singleBinary.replicas: 1`, and replication factor 1. Chart 12 renamed the deployment-mode value from `SingleBinary` to `Monolithic`, so new bases must use the new name. Pin the monolithic `tempo` chart `2.2.3` (app `2.10.7`) with `replicas: 1`, not `tempo-distributed`; pin Alloy chart `1.11.1` (app `v1.18.1`) with `controller.type: daemonset` for logs. Sources: [Loki monolithic guide and repository migration](https://grafana.com/docs/loki/latest/setup/install/helm/install-monolithic/), [Tempo chart guidance](https://grafana.com/docs/tempo/latest/set-up-for-tracing/setup-tempo/deploy/kubernetes/helm-chart/), [Grafana Community chart index](https://grafana-community.github.io/helm-charts/index.yaml), and [Grafana chart index](https://grafana.github.io/helm-charts/index.yaml).

26. **2026-08-09 — Flux bootstrap, source maturity, and k3s baseline:** Pin Flux `v2.9.4` and bootstrap platform-infra with the Flux CLI's GitHub/provider bootstrap flow. Continue consuming baukit bases through a tag-pinned `GitRepository` because git preserves the existing release and review path, not because OCI is immature: `OCIRepository` is already GA at `source.toolkit.fluxcd.io/v1` and is a valid later distribution alternative. Pin the k3s `stable` channel currently resolving to `v1.36.3+k3s1`; disable bundled Traefik on every server via `disable: [traefik]` in `/etc/rancher/k3s/config.yaml`, the supported config-file equivalent of `--disable=traefik`. Sources: [Flux 2.9.4](https://github.com/fluxcd/flux2/releases/tag/v2.9.4), [Flux bootstrap](https://fluxcd.io/flux/installation/bootstrap/), [`OCIRepository` v1 API](https://fluxcd.io/flux/components/source/api/v1/), [k3s stable channel](https://update.k3s.io/v1-release/channels/stable), [k3s configuration file](https://docs.k3s.io/installation/configuration), and [disabling packaged Traefik](https://docs.k3s.io/networking/networking-services).

27. **2026-08-09 — Hosted dependency automation and GHCR retention:** The hosted Mend Renovate Community Cloud remains free for unlimited public and private repositories, including personal private repositories, but it is constrained to one concurrent job per organization, a four-hour active-repository cadence, 1 vCPU, 3 GB RAM, 15 GB disk, and a 30-minute job timeout. Keep the hosted choice, with the monorepo timeout as the operational risk. GitHub Free still documents a nominal Packages allowance of 500 MB storage and 1 GB/month transfer, but GitHub explicitly says Container registry storage and bandwidth are currently free and promises at least one month's notice before charging; therefore decision 20's quota watch is prudent but there is no current GHCR storage charge to optimize against. GHCR versions can be deleted manually or through the REST API, and `GITHUB_TOKEN` deletion/restoration from Actions is public preview; there is no documented native age/count retention policy, so I4 should implement conservative API-based pruning that protects deployed digests and retains a fixed recent set. Sources: [Mend-hosted Renovate overview and limits](https://docs.renovatebot.com/mend-hosted/overview/), [Renovate job scheduling](https://docs.renovatebot.com/mend-hosted/job-scheduling/), [GitHub Packages billing](https://docs.github.com/en/billing/concepts/product-billing/github-packages), [package deletion/restoration](https://docs.github.com/en/packages/learn-github-packages/deleting-and-restoring-a-package), and [Packages REST API](https://docs.github.com/en/rest/packages/packages).

28. **2026-08-09 — Kargo remains deferred on fit, not maturity:** Kargo is at `v1.11.0`, Apache-2.0 licensed, has a regular approximately six-week minor-release roadmap, and its public repository shows roughly 3.5k stars and 410 forks; vendor-published adopters include Deutsche Telekom, JumpCloud, and Cisco ThousandEyes. This is sufficient maturity evidence, but it does not change decision 20: two environments and one initial application do not justify another controller, CRD model, and UI. Revisit at the already recorded threshold of at least three deployed apps or measured PR-promotion friction. Sources: [Kargo 1.11.0 release](https://github.com/akuity/kargo/releases/tag/v1.11.0), [repository and Apache-2.0 license](https://github.com/akuity/kargo), [roadmap](https://docs.kargo.io/roadmap), and [published adoption examples](https://akuity.io/blog/kargo-custom-steps-gitops-promotion).
